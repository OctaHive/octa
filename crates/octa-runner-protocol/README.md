...
Protocol v3 adds an optional, job-scoped `CacheSessionSpec` containing explicit
operator-selected L1 capacity, local CAS placement, access mode, namespace, and
an exact Native or OCI runtime identity. A remote session contains only an
HTTPS endpoint and token-file path, never bearer token contents. See the repository
