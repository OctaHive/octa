...
async fn load_runtime(
  ...
  config.cache = request
    ...
    .as_ref()
    .map(|cache| {
      let local = RuntimeCacheConfig::local(
        cache.mode,
      ...
      match &cache.remote {
        Some(remote) => local.with_remote(
          &remote.endpoint,
          &remote.token_file,
          remote.ca_certificate_file.as_deref(),
          Duration::from_secs(remote.request_timeout_seconds),
          remote.max_parallel_transfers.get(),
        ),
        None => Ok(local),
      }
    })
